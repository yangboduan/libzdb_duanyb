...
/* added by duanyb 2018-11-3*/
unsigned long long ResultSet_getULLong(T R, int columnIndex) {
	assert(R);
        const char *s = R->op->getString(R->D, columnIndex);
	return s ? Str_parseULLong(s) : 0;
}
...
/* added by duanyb 2018-11-3*/
unsigned long long ResultSet_getULLongByName(T R, const char *columnName) {
	assert(R);
	return ResultSet_getULLong(R, _getIndex(R, columnName));
}
